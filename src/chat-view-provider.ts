...
export default class ChatViewProvider implements vscode.WebviewViewProvider {
	...
	constructor(private context: vscode.ExtensionContext) {
		this.subscribeToResponse = vscode.workspace.getConfiguration("ollama-developer-assistant").get("chat.showNotification") || false;
		this.autoScroll = !!vscode.workspace.getConfiguration("ollama-developer-assistant").get("chat.autoScroll") || true;
		this.model = vscode.workspace.getConfiguration("ollama-developer-assistant").get("chat.model") as string;
	...
	public async sendApiRequest(prompt: string, options: { command: string, code?: string, previousAnswer?: string, language?: string; }) {
		...
		try {
			...
			console.log(answer)
			// ({ text: this.response, id: this.conversationId, parentMessageId: this.messageId } = answer);
...
`;
	...
	private logEvent(eventName: string, properties?: {}): void {
		...
		console.debug(eventName, { "chatgpt.model": this.model || "unknown", ...properties }, { "chatgpt.questionCounter": this.questionCounter });
	}
	...
	private logError(eventName: string): void {
		...
		console.error(eventName, { "chatgpt.model": this.model || "unknown" }, { "chatgpt.questionCounter": this.questionCounter });
	}
